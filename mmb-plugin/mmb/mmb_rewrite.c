...
u32 mmb_rewrite(mmb_conn_table_t *mct, vlib_main_t *vm, mmb_rule_t *rule,
  ...
  if (PREDICT_FALSE(rule->shuffle || rule->map)) {
...
mmb_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node,
  ...
  mmb_tcp_options_t tcp_options0;
  ...
  while (n_left_from > 0)
  {
    ...
    while (n_left_from > 0 && n_left_to_next > 0)
    {
      ...
      vec_foreach(rule_index0, rule_indexes0) {
         ...
         if (PREDICT_FALSE(ri0->opts_in_targets)) { // TODO: && !tcpo0 instead of re-parsing
             if (is_ip6)
      ...
      if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE)
         ...
         mmb_trace_ip_packet(vm, b0, node, p0, next0, is_ip6);
      }
