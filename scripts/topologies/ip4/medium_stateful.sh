...
sudo vppctl -s /run/vpp/cli-vpp1.sock set int ip address memif0/0 10.202.129.13/24

sudo vppctl -s /run/vpp/cli-vpp1.sock ip route add 10.0.0.0/24 via 10.202.129.2 memif0/0
sudo vppctl -s /run/vpp/cli-vpp1.sock ip route add 10.202.129.0/24 via 10.202.129.2 memif0/0
sudo vppctl -s /run/vpp/cli-vpp1.sock ip route add 20.55.55.0/24 via 10.202.129.2 memif0/0
sudo vppctl -s /run/vpp/cli-vpp1.sock ip route add 30.0.0.0/8 via 10.202.129.2 memif0/0
...
sudo vppctl -s /run/vpp/cli-vpp2.sock set int ip address memif0/0 10.202.129.2/24
...
sudo vppctl -s /run/vpp/cli-vpp2.sock set int ip address memif1/0 30.0.3.1/24
...
sudo vppctl -s /run/vpp/cli-vpp2.sock ip route add 10.100.100.0/24 via 10.202.129.13 memif0/0
sudo vppctl -s /run/vpp/cli-vpp2.sock ip route add 10.202.129.0/24 via 10.202.129.13 memif0/0
sudo vppctl -s /run/vpp/cli-vpp2.sock ip route add 10.0.0.0/24 via 30.0.3.3 memif1/0
sudo vppctl -s /run/vpp/cli-vpp2.sock ip route add 20.55.55.0/24 via 30.0.3.3 memif1/0
sudo vppctl -s /run/vpp/cli-vpp2.sock ip route add 30.101.0.0/16 via 30.0.3.3 memif1/0
sudo vppctl -s /run/vpp/cli-vpp2.sock ip route add 30.111.2.0/24 via 30.0.3.3 memif1/0
sudo vppctl -s /run/vpp/cli-vpp2.sock ip route add 30.0.0.0/16 via 30.0.3.3 memif1/0
...
sudo vppctl -s /run/vpp/cli-vpp3.sock set int ip address memif0/0 30.0.3.3/24
sudo vppctl -s /run/vpp/cli-vpp3.sock create memif socket /run/vpp/memif-vpp3vpp5 master
sudo vppctl -s /run/vpp/cli-vpp3.sock set int state memif1/0 up
sudo vppctl -s /run/vpp/cli-vpp3.sock set int ip address memif1/0 30.0.5.1/24

sudo vppctl -s /run/vpp/cli-vpp3.sock ip route add 10.100.100.0/24 via 30.0.3.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp3.sock ip route add 10.202.129.0/24 via 30.0.3.1 memif0/0
#sudo vppctl -s /run/vpp/cli-vpp3.sock ip route add 30.1.0.0/16 via 30.0.3.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp3.sock ip route add 10.0.0.0/24 via 30.0.5.7 memif1/0
sudo vppctl -s /run/vpp/cli-vpp3.sock ip route add 20.55.55.0/24 via 30.0.5.7 memif1/0
sudo vppctl -s /run/vpp/cli-vpp3.sock ip route add 30.101.0.0/16 via 30.0.5.7 memif1/0
sudo vppctl -s /run/vpp/cli-vpp3.sock ip route add 30.111.2.0/24 via 30.0.5.7 memif1/0
sudo vppctl -s /run/vpp/cli-vpp3.sock mmb enable memif0/0
sudo vppctl -s /run/vpp/cli-vpp3.sock mmb enable memif1/0
...
sudo vppctl -s /run/vpp/cli-vpp5.sock create memif socket /run/vpp/memif-vpp3vpp5 slave
sudo vppctl -s /run/vpp/cli-vpp5.sock set int state memif0/0 up
sudo vppctl -s /run/vpp/cli-vpp5.sock set int ip address memif0/0 30.0.5.7/24
...
sudo vppctl -s /run/vpp/cli-vpp5.sock set int ip address memif1/0 20.55.55.1/24

#sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 30.1.0.0/16 via 30.1.2.5 memif0/0
sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 10.100.100.0/24 via 30.0.5.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 10.202.129.0/24 via 30.0.5.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 30.0.0.0/16 via 30.0.5.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 10.0.0.0/24 via 20.55.55.6 memif1/0
sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 20.55.55.0/24 via 20.55.55.6 memif1/0
sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 30.101.0.0/16 via 20.55.55.6 memif1/0
sudo vppctl -s /run/vpp/cli-vpp5.sock ip route add 30.111.2.0/24 via 20.55.55.6 memif1/0
sudo vppctl -s /run/vpp/cli-vpp5.sock mmb enable memif0/0
sudo vppctl -s /run/vpp/cli-vpp5.sock mmb enable memif1/0
...
sudo vppctl -s /run/vpp/cli-vpp6.sock set int ip address memif0/0 20.55.55.6/24
sudo vppctl -s /run/vpp/cli-vpp6.sock create memif socket /run/vpp/memif-vpp6vpp7 master
sudo vppctl -s /run/vpp/cli-vpp6.sock set int state memif1/0 up
sudo vppctl -s /run/vpp/cli-vpp6.sock set int ip address memif1/0 30.101.2.3/16
sudo vppctl -s /run/vpp/cli-vpp6.sock mmb enable memif0/0
sudo vppctl -s /run/vpp/cli-vpp6.sock mmb enable memif1/0

sudo vppctl -s /run/vpp/cli-vpp6.sock ip route add 10.100.100.0/24 via 20.55.55.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp6.sock ip route add 10.202.129.0/24 via 20.55.55.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp6.sock ip route add 30.0.0.0/16 via 20.55.55.1 memif0/0
#sudo vppctl -s /run/vpp/cli-vpp6.sock ip route add 30.1.0.0/16 via 20.55.55.1 memif0/0
sudo vppctl -s /run/vpp/cli-vpp6.sock ip route add 10.0.0.0/24 via 30.101.0.6 memif1/0
sudo vppctl -s /run/vpp/cli-vpp6.sock ip route add 30.111.2.0/24 via 30.101.0.6 memif1/0
...
sudo vppctl -s /run/vpp/cli-vpp7.sock create memif socket /run/vpp/memif-vpp6vpp7 slave
sudo vppctl -s /run/vpp/cli-vpp7.sock set int state memif0/0 up
sudo vppctl -s /run/vpp/cli-vpp7.sock set int ip address memif0/0 30.101.0.6/16
sudo vppctl -s /run/vpp/cli-vpp7.sock create memif socket /run/vpp/memif-vpp7vpp8 master
sudo vppctl -s /run/vpp/cli-vpp7.sock set int state memif1/0 up
sudo vppctl -s /run/vpp/cli-vpp7.sock set int ip address memif1/0 30.111.2.3/24
sudo vppctl -s /run/vpp/cli-vpp7.sock mmb enable memif0/0
sudo vppctl -s /run/vpp/cli-vpp7.sock mmb enable memif1/0

sudo vppctl -s /run/vpp/cli-vpp7.sock ip route add 10.100.100.0/24 via 30.101.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp7.sock ip route add 10.202.129.0/24 via 30.101.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp7.sock ip route add 30.0.0.0/16 via 30.101.2.3 memif0/0
#sudo vppctl -s /run/vpp/cli-vpp7.sock ip route add 30.1.0.0/16 via 30.101.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp7.sock ip route add 20.55.55.0/24 via 30.101.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp7.sock ip route add 30.101.2.0/24 via 30.101.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp7.sock ip route add 10.0.0.0/24 via 30.111.2.2 memif1/0
...
sudo vppctl -s /run/vpp/cli-vpp8.sock create memif socket /run/vpp/memif-vpp7vpp8 slave
sudo vppctl -s /run/vpp/cli-vpp8.sock set int state memif0/0 up
sudo vppctl -s /run/vpp/cli-vpp8.sock set int ip address memif0/0 30.111.2.2/24

sudo vppctl -s /run/vpp/cli-vpp8.sock ip route add 10.100.100.0/24 via 30.111.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp8.sock ip route add 10.202.129.0/24 via 30.111.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp8.sock ip route add 30.0.0.0/16 via 30.111.2.3 memif0/0
#sudo vppctl -s /run/vpp/cli-vpp8.sock ip route add 30.1.0.0/16 via 30.111.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp8.sock ip route add 20.55.55.0/24 via 30.111.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp8.sock ip route add 30.101.2.0/24 via 30.111.2.3 memif0/0
sudo vppctl -s /run/vpp/cli-vpp8.sock ip route add 30.111.2.0/24 via 30.111.2.3 memif0/0
...
sudo vppctl -s /run/vpp/cli-vpp8.sock mmb enable memif0/0
